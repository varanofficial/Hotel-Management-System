...
Public Class frmCheckout
    ...
    Private Sub txtRoomNumber_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtRoomNumber.TextChanged
        ...
        Else
            ...
            rs = New SqlDataAdapter("SELECT * FROM tblRoom WHERE RoomNumber = " & txtRoomNumber.Text & "", con)
            rs.Fill(dt)
    ...
    Private Sub bttnCheckout_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bttnCheckout.Click
        ...
        Else
            ...
            Else
                ...
                If out = vbYes Then
                    con.Open()
                    Dim update_trans As New SqlCommand("UPDATE tblTransaction SET Remarks = 'Checkout' WHERE TransID = " & lblTransID.Text & "", con)
                    update_trans.ExecuteNonQuery()

                    Dim update_guest As New SqlCommand("UPDATE tblGuest SET Remarks = 'Available' WHERE ID = " & lblGuestID.Text & "", con)
                    update_guest.ExecuteNonQuery()

                    Dim update_room As New SqlCommand("UPDATE tblRoom SET Status = 'Available' WHERE RoomNumber = " & txtRoomNumber.Text & "", con)
