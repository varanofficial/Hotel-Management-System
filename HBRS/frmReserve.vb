...
Public Class frmReserve
    ...
    Public Sub transID()
        ...
        rs = New SqlDataAdapter("SELECT * FROM tblTransaction ORDER BY TransID DESC", con)
        rs.Fill(dt)
    ...
    Private Sub pop_discount()
        ...
        rs = New SqlDataAdapter("SELECT * FROM tblDiscount", con)
        rs.Fill(dt)
    ...
    Private Sub bttnReserve_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bttnReserve.Click
        ...
        Else
            ...
            If a = vbYes Then
                con.Open()
                Dim checkin As New SqlCommand("INSERT INTO tblTransaction(GuestID,RoomNum,CheckInDate,CheckOutDate,ReservationDate,NoOfChild,NoOfAdult,AdvancePayment,DiscountID,Remarks,Status) values ('" &
                                                lblGuestID.Text & "','" &
                                                txtRoomNumber.Text & "','" &
                                                dtCheckInDate.Text & "','" &
                                                dtCheckOutDate.Text & "','" &
                ...
                Dim update_guest As New SqlCommand("UPDATE tblGuest SET Remarks = 'Reserve' WHERE ID = " & lblGuestID.Text & "", con)
                update_guest.ExecuteNonQuery()

                Dim update_room As New SqlCommand("UPDATE tblRoom SET Status = 'Reserve' WHERE RoomNumber = " & txtRoomNumber.Text & "", con)
    ...
    Private Sub cboDiscount_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboDiscount.TextChanged
        ...
        rs = New SqlDataAdapter("SELECT * FROM tblDiscount WHERE DiscountType = '" & cboDiscount.Text & "'", con)
        rs.Fill(dt)
    ...
    Private Sub lvlreserve_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lvlreserve.Click
        ...
        Dim dt As New DataTable("tblTransaction")
        Dim rs As SqlDataAdapter
        rs = New SqlDataAdapter("SELECT * FROM tblTransaction", con)
        rs.Fill(dt)
        Dim indx As Integer
        For indx = 0 To dt.Rows.Count - 1
    ...
    Private Sub bttnCheckin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bttnCheckin.Click
        ...
        If check_in = vbYes Then
            con.Open()
            Dim update_trans As New SqlCommand("UPDATE tblTransaction SET Remarks = 'Checkin' WHERE TransID = " & id & "", con)
            update_trans.ExecuteNonQuery()

            Dim update_guest As New SqlCommand("UPDATE tblGuest SET Remarks = 'Checkin' WHERE ID = " & guest_id & "", con)
            update_guest.ExecuteNonQuery()

            Dim update_room As New SqlCommand("UPDATE tblRoom SET Status = 'Occupied' WHERE RoomNumber = " & room_num & "", con)
            update_room.ExecuteNonQuery()
            con.Close()
            display_reserve()
    ...
    Private Sub bttnCancelReserve_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bttnCancelReserve.Click
        ...
        If check_in = vbYes Then
            ...
            Dim update_trans As New SqlCommand("UPDATE tblTransaction SET Remarks = 'Cancelled' WHERE TransID = " & id & "", con)
            update_trans.ExecuteNonQuery()

            Dim update_guest As New SqlCommand("UPDATE tblGuest SET Remarks = 'Available' WHERE ID = " & guest_id & "", con)
            update_guest.ExecuteNonQuery()

            Dim update_room As New SqlCommand("UPDATE tblRoom SET Status = 'Available' WHERE RoomNumber = " & room_num & "", con)
            update_room.ExecuteNonQuery()
