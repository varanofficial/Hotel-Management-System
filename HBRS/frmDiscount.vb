...
Public Class frmDiscount
    ...
    Private Sub bttnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bttnSave.Click
        ...
        If a = vbYes Then
            If update_discount = False Then
                con.Open()
                Dim save As New SqlCommand("INSERT INTO tblDiscount(DiscountType,DiscountRate,Status) values ('" & Trim(txtType.Text) & "','" & Trim(txtRate.Text) & "','Active')", con)
                save.ExecuteNonQuery()
                con.Close()
            Else
                con.Open()
                Dim update As New SqlCommand("UPDATE tblDiscount SET DiscountType = '" & Trim(txtType.Text) & "',DiscountRate = '" & Val(Trim(txtRate.Text)) / 100 & "' WHERE ID = '" & id & "'", con)
    ...
    Private Sub lvlDiscount_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lvlDiscount.SelectedIndexChanged
