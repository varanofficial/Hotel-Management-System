...
Public Class frmCheckOutList
    ...
    Private Sub frmCheckOutList_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        rs = New SqlDataAdapter("Select * from tblTransaction, tblGuest, tblDiscount, tblRoom WHERE tblTransaction.GuestID = tblGuest.ID AND tblTransaction.DiscountID = tblDiscount.ID AND tblTransaction.RoomNum = tblRoom.RoomNumber AND tblTransaction.Remarks = 'Checkout' AND tblTransaction.Status = 'Active'", con)
        rs.Fill(Dt)
    ...
    Private Sub lvlcheckin_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lvlcheckin.SelectedIndexChanged
